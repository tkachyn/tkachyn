<div align="left">

# Tim K. ( @tkachyn )
`automation • problem-solving • productivity tools`


</div>


## about

```txt
name      : Tim K 
alias     : tkachyn
location  : toronto, on
status    : first-year computer science student @ university of toronto
```

i build fast, intentional tools that solve real problems.

<p align="left">
  <img src="https://readmeme.eu.cc/api/time.svg?theme=ink&timezone=America%2FToronto&timeFormat=12h&showSeconds=0&showDate=0&showDay=0&label=Toronto" alt="Current local time in Toronto" />
</p>

---


## stack

<p align="left">
  <img src="https://img.shields.io/badge/go-111111?style=for-the-badge&logo=go&logoColor=white" />
  <img src="https://img.shields.io/badge/c%23-111111?style=for-the-badge&logo=csharp&logoColor=white" />
  <img src="https://img.shields.io/badge/java-111111?style=for-the-badge&logo=openjdk&logoColor=white" />
  <img src="https://img.shields.io/badge/javascript-111111?style=for-the-badge&logo=javascript&logoColor=white" />
  <img src="https://img.shields.io/badge/python-111111?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/c%2B%2B-111111?style=for-the-badge&logo=cplusplus&logoColor=white" />
</p>

```txt
building : workflow tools · backend tools · purpose-built software
interested : systems · performance · developer tooling
```

---

## contact me!

<p align="left">
  <a href="mailto:tim.kachynskyy@gmail.com">
    <img src="https://img.shields.io/badge/email-111111?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>

  <a href="https://github.com/tkachyn">
    <img src="https://img.shields.io/badge/github-111111?style=for-the-badge&logo=github&logoColor=white" />
  </a>

</p>
